    ...
    <div class="chat__messages" role="log" aria-live="polite" aria-label="Conversation messages" tabindex="0" data-chat-messages>
        ...
        <?php if ($messages === []): ?>
            <div class="chat__empty" data-chat-empty>
                ...
                <p class="util-muted">Answers come only from indexed global rule documents, with sources.</p>
                    ...
                    <article class="chat-msg chat-msg--<?= $isUser ? 'user' : 'assistant' ?>" data-message-id="<?= $message->id ?>">
                        ...
                        <?php if ($isEditable): ?>
                            ...
                            <form method="post" class="chat-msg__edit" data-edit-form
                                  action="<?= Html::encode($urlGenerator->generate('admin.rule-chat.message.edit', ['conversationId' => $conversation->id, 'messageId' => $message->id])) ?>">
                                <?= $csrfField ?>
                        ...
                        <?php if ($isRetry): ?>
                            <form method="post" class="chat-msg__retry" data-retry-form
                                  action="<?= Html::encode($urlGenerator->generate('admin.rule-chat.message.regenerate', ['conversationId' => $conversation->id, 'messageId' => $message->id])) ?>">
                                <?= $csrfField ?>
