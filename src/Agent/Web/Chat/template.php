                    ...
                    <article class="chat-msg chat-msg--<?= $isUser ? 'user' : 'assistant' ?>" data-message-id="<?= $message->id ?>">
                                    ...
                                    <?php foreach ($message->citations as $citation): ?>
                                        ...
                                        <?php if ($conversation !== null): ?>
                                            <button type="button" class="chat-chip chat-chip--source"
                                                    data-source-url="<?= Html::encode($urlGenerator->generate('agent.chat.message.source', ['slug' => $slug, 'conversationId' => $conversation->id, 'messageId' => $message->id, 'documentId' => $citation->documentId])) ?>"
                        ...
                        <?php if ($message->isAssistant() && $conversation !== null): ?>
                            <?= $this->render(ChatPartials::scorePanel(), [
                                ...
                                'scoreUrl' => $urlGenerator->generate('agent.chat.message.score', ['slug' => $slug, 'conversationId' => $conversation->id, 'messageId' => $message->id]),
                                'dismissUrl' => $urlGenerator->generate('agent.chat.message.score.dismiss', ['slug' => $slug, 'conversationId' => $conversation->id, 'messageId' => $message->id]),
                                'csrfField' => $csrfField,
                            ]) ?>
                        <?php endif; ?>
                    </article>
        ...
        <?php elseif ($chatReady): ?>
            <form method="post" action="<?= Html::encode($postUrl) ?>" class="chat__form">
                ...
                <input class="field__control chat__input" type="text" id="chat-question" name="question"
                       maxlength="<?= $maxQuestionLength ?>" placeholder="Ask a question about this store…"
