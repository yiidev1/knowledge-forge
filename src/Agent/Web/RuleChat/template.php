...
$viewRulesUrl = $urlGenerator->generate('agent.rule-chat.sources.rules');
                    ...
                    <article class="chat-msg chat-msg--<?= $isUser ? 'user' : 'assistant' ?>" data-message-id="<?= $message->id ?>">
                                    ...
                                    <?php foreach ($message->citations as $citation): ?>
                                        ...
                                        <?php if ($conversation !== null): ?>
                                            <button type="button" class="chat-chip chat-chip--source"
                                                    data-source-url="<?= Html::encode($urlGenerator->generate('agent.rule-chat.message.source', ['conversationId' => $conversation->id, 'messageId' => $message->id, 'documentId' => $citation->documentId])) ?>"
                        ...
                        <?php if ($message->isAssistant() && $conversation !== null): ?>
                            <?= $this->render(ChatPartials::scorePanel(), [
                                ...
                                'scoreUrl' => $urlGenerator->generate('agent.rule-chat.message.score', ['conversationId' => $conversation->id, 'messageId' => $message->id]),
                                'dismissUrl' => $urlGenerator->generate('agent.rule-chat.message.score.dismiss', ['conversationId' => $conversation->id, 'messageId' => $message->id]),
                                'csrfField' => $csrfField,
                            ]) ?>
                        <?php endif; ?>
                    </article>
