...
<div class="filter-bar" role="group" aria-label="Filter by source status and agent access">
    ...
    <?php foreach (StoreChatAvailabilityFilter::cases() as $option): ?>
        <?php $isActive = $option === $chatAvailability ? ' filter-chip--active' : ''; ?>
        <a class="filter-chip<?= $isActive ?>" href="<?= Html::encode($dirUrl(['chat' => $option->value, 'page' => 1])) ?>"><?= Html::encode($option->label()) ?></a>
    <?php endforeach; ?>
</div>
